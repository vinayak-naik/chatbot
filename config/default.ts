export default {
  port: 1337,
  host: "localhost",
  mongodbLocal: "mongodb://localhost:27017/rest-api",
  mongodbAtlas: "mongodb+srv://vinayak:vinayak@cluster0.dsnun.mongodb.net/chatbot?retryWrites=true&w=majority",
  saltWorkFactor: 10,
  accessTokenTtl: "1d",
  refreshTokenTtl: "1y",
  senderEmail: "vinayaknaik4321@outlook.com",
  senderEmailPassword: "Vinayak*503",
  receiverEmail1: "vinayaknaik2403@gmail.com",
  receiverEmail2: "vinayaknaik7259@gmail.com",
  privateKey: `-----BEGIN RSA PRIVATE KEY-----
  
  -----END CERTIFICATE-----`,
};
